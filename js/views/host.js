  ...
  function render() {
    ...
    root.innerHTML = `
      <div class="card">
        ...
        <p class="dim">Phase: <strong>${ph}</strong> \u00b7 round ${n + 1}/${N_ROUNDS} \u00b7 ${humans.length} humans${(S.state && S.state.tut) ? " \u00b7 tutorial step " + S.state.tut + "/5 on screen" : ""} \u00b7 rollover ${fmt((S.state && S.state.rollover) || 0)} \u00b7 build ${BUILD}</p>
        ${!isHost() ? `<button class="big" id="claim">Claim host on this device</button>` : `
          ...
          <div class="btnrow">
            ...
            <button id="tut" ${ph === "lobby" ? "" : "disabled"}>Tutorial: ${((S.state && S.state.tut) || 0) >= 5 ? "end, back to QR" : "show step " + (((S.state && S.state.tut) || 0) + 1) + "/5"}</button>
    ...
    if (q("#tut")) q("#tut").onclick = async () => {
      ...
      const nx = cur >= 5 ? 0 : cur + 1;
      await update(gref(), { "state/tut": nx });
      log(nx ? `Tutorial step ${nx}/5 on the big screen.` : "Tutorial ended. QR is back on screen.");
    };
